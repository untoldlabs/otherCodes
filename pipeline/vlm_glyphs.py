"""
vlm_glyphs.py — Symbolic glyph-focused VLM detection.

PROMPT PHILOSOPHY — GLYPHS / SYMBOLS / MOTIFS
===============================================
Focused specifically on symbolic motifs and glyphs — elements that carry
meaning beyond pure geometry: hearts, arrows, stars, drips, peace signs,
eyes (as isolated symbols), accent dots, S-curves, and rarer motifs
like prehistoric marks (claviform, tectiform, penniform, etc.).

  Pass 1  : Re-examines the image focused ONLY on symbolic glyphs/motifs.
...
BIAS PROFILE
============
Higher recall for symbolic/glyph content. May find symbolic meanings in
abstract strokes. Compare certain_* against vlm.py sym_* to calibrate.

Outputs:
  data/vlm_scores_glyphs.csv  — one row per mark
  data/vlm_glyphs/            — annotated report card images

Usage:
    conda activate othercodes
    python3 pipeline/vlm_glyphs.py --project /path/to/project
    python3 pipeline/vlm_glyphs.py --project ~/proj --stems IMG_0001
"""
...
def _build_pass1_prompt() -> str:
    """Pass 1 — focused look at SYMBOLIC GLYPHS and MOTIFS."""
    return """You are analysing a segmented graffiti tag or handmade mark on a white background.

Look carefully at this image. Focus SPECIFICALLY on symbolic motifs and glyphs —
elements that carry symbolic meaning beyond pure geometry:

  Hearts / cordiform shapes
  Arrows (with or without shafts), arrowheads
  Stars (five-pointed), asterisks, starburst shapes
  Paint drips, teardrops, dribbles
  Peace signs (circle with vertical + two diagonals)
  Eyes as isolated symbolic motifs (not as part of a face)
  Sun symbols, wheels, radial motifs
  Snake / S-curve / serpentiform shapes
  Small floating accent dots or marks
  Feather / penniform shapes
  Heart-with-arrow (love symbol)
  Crown shapes, flower shapes, leaf shapes (as glyphs)
  Ladder-like forms, fan shapes, kidney shapes
  Any prehistoric-style mark or petroglyph-like symbol

If you see ANY of these, name them precisely.
Do NOT describe pure geometric primitives (circle, triangle) or faces — focus ONLY on symbolic glyphs.

Respond with ONLY a JSON object (no markdown, no explanation):
{
  "description": "Precise list of every symbolic glyph or motif present, e.g. 'heart, arrow, paint drip, accent dot'. Or 'none detected'.",
  "letters": [],
  "words": [],
  "text_script": "none"
}
...
- Look at the whole image including corners and small floating marks.
"""


def _build_pass2_prompt(p1_description: str) -> str:
    return f"""You just examined this graffiti mark and identified these symbolic glyphs:
...
(not just your previous description), decide what is:
  ...
  CERTAIN  — symbolic glyphs/motifs clearly and unambiguously present
  AMBIGUOUS — glyphs that might be present but are unclear, partial,
               or could be read as something else

Focus on: hearts, arrows, stars, drips, peace signs, eyes (as symbols),
sun wheels, S-curves, accent dots, feathers, fans, leaves, prehistoric marks.

Respond with ONLY a JSON object:
{{
  "certain": "comma-separated list of clearly present glyphs, e.g. 'heart, drip, accent dot'",
  "ambiguous": "comma-separated list of uncertain glyphs, e.g. 'arrow, peace sign'"
}}
...
def render_report(stem: str, img_path: Path,
    ...
    txt(f"{stem}  [glyphs]", colour=(255, 255, 100))
    gap()
    txt("PASS 1 — GLYPH/MOTIF SCAN:", colour=(160, 160, 255))
