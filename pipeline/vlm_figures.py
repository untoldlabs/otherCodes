"""
vlm_figures.py — Figurative-focused VLM symbol detection.

PROMPT PHILOSOPHY — FIGURES / FIGURATIVE
=========================================
This script uses a two-pass architecture focused specifically on figurative
content: faces, people, animals, characters, hands, skulls, masks, and any
anthropomorphic or zoomorphic forms.

  Pass 1  : Re-examines the image with a prompt focused ONLY on figurative
             content. The model looks afresh at the image — not building on
             prior words — with attention directed toward faces and figures.

  Pass 2  : Re-examines the image AND Pass 1 output. Consolidates into two
             tiers:
               certain  — elements clearly and unambiguously present
               ambiguous — elements that might be present but are unclear,
                           partial, or uncertain

  Grep    : Runs separately on "certain" and "ambiguous" text.
             → certain_{key}  columns in CSV (higher trust)
             → ambiguous_{key} columns in CSV (lower trust)

BIAS PROFILE
============
Higher recall for figurative content than vlm.py (neutral). Trade-off:
more likely to report faces/figures when they are borderline. Compare
certain_{key} against vlm.py sym_{key} to calibrate.

WARNING: Confirmation bias risk. A focused prompt will find things that
a neutral prompt misses — but it may also find things that aren't there.
Trust certain_* more than ambiguous_*. Cross-check against vlm.py output.

Outputs:
  data/vlm_scores_figures.csv  — one row per mark
  data/vlm_figures/            — annotated report card images

Usage:
    conda activate othercodes
    python3 pipeline/vlm_figures.py --project /path/to/project
    python3 pipeline/vlm_figures.py --project ~/proj --stems IMG_0001
"""
...
def _build_pass1_prompt() -> str:
    """Pass 1 — focused look at FIGURATIVE content only."""
    return """You are analysing a segmented graffiti tag or handmade mark on a white background.

Look carefully at this image. Focus SPECIFICALLY on figurative content:
faces, heads, portraits, eyes, mouths, noses, ears, skulls, masks, smileys,
people, bodies, figures, hands, fingers, animals, cartoon characters,
anthropomorphic shapes, humanoid forms, caricatures, silhouettes of figures.

If you see ANY of these, describe them precisely.
If none are present, say so clearly.

Do NOT use vague terms — name the actual elements.

Respond with ONLY a JSON object (no markdown, no explanation):
{
  "description": "Precise description of every figurative element present, or 'none detected'.",
  "letters": [],
  "words": [],
  "text_script": "none"
}
...
Rules:
- "letters": every readable character (e.g. ["R", "U", "L"]).
...
def _build_pass2_prompt(p1_description: str) -> str:
    ...
    return f"""You just examined this graffiti mark and found the following figurative content:

"{p1_description}"

Now look at the image again carefully. Based on what you see in the image right now
(not just your previous description), decide what is:

  CERTAIN  — clearly and unambiguously present in the image
  AMBIGUOUS — possibly present but unclear, partial, distorted, or uncertain

Focus on: faces, heads, skulls, masks, eyes-and-mouth combinations, people,
figures, hands, animals, cartoon characters, anthropomorphic or humanoid forms.

Respond with ONLY a JSON object:
{{
  "certain": "comma-separated list of elements clearly present, e.g. 'face, skull'",
  "ambiguous": "comma-separated list of uncertain elements, e.g. 'eye, partial figure'"
}}

Leave a field as an empty string "" if nothing belongs there.
Do NOT include geometric shapes — only figurative / representational content.
"""
...
def render_report(stem: str, img_path: Path,
    ...
    """Report card with CERTAIN (green) and AMBIGUOUS (amber) sections."""
    ...
    # Overlay detections — certain in bright, ambiguous dimmer
    ...
    txt(f"{stem}  [figures]", colour=(255, 255, 100))
    gap()

    txt("PASS 1 — FIGURATIVE SCAN:", colour=(160, 160, 255))
...
def process_mark(stem: str, model: str) -> dict | None:
    ...
    try:
        ...
        # Pass 1 — focused figurative scan
        print(f"  {stem}  — pass 1 (figures)…")
        ...
        # Pass 2 — re-examine image, output certain / ambiguous
    ...
    render_report(stem, img_path,
                  p1_desc, certain_text, ambiguous_text,
                  certain_dets, ambiguous_dets,
                  letters, words, script)
